...
const create = (request: Request, response: Response, next: NextFunction) => {
  ...
  if (!data.celebrity || !data.stalker || !data.location) {
    return response.status(422).json({error: {status: 422, msg: "The celebrity, stalker, and location fields may not be blank. The record was not saved."}});
  } else {
...
const update = (request: Request, response: Response, next: NextFunction) => {
  ...
  if (dataValuesArray.map(value => Boolean(value)).includes(false)) {
    return response.status(422).json({error: {status: 422, msg: "The celebrity, stalker, location, and date fields may not be blank. The record was not updated."}});
  } else if (data.id) {
    return response.status(422).json({error: {status: 422, msg: "Database record id's are unique and cannot be changed. The record was not updated."}});
  } else {
    return knex("sightings")
      .returning("*")
